    ...
    <div class="match_setting">
        <Submenu name="19">
            ...
            <div style="margin-left: 25px;margin-top: -15px;">
                ...
                <i-table :columns="columns2" :data="colorMatchData"></i-table>
            </div>
        </Submenu>
        <Modal :loading="loading" v-model="colorMatchModal" title="自定义配色" :width="30" @on-ok="addColorMatch" @on-cancel="onCancel">
            <div style="padding-right: 50px">
                <i-form :model="colorMatch" label-colon :label-width="90">
                    <form-item label="系列" v-if="selectedChartType.indexOf('gauge')==-1">
                        <i-select v-model="colorMatch.name" style="width:100%"  placeholder="请选择配置系列">
                            <i-option v-for="(item, index) in seriesList" :index="index" :value="item">{{item}}</i-option>
                        </i-select>
                    </form-item>
        ...
        methods: {
            ...
            onColorChange(){
                ...
                for(let item of arr){
                    ...
                    let temp = this.colorMatchData.filter(c=>{
                    ...
                    if(this.selectedChartType.indexOf('radar')>=0) {
                        ...
                        if (temp && temp.length > 0) {
                            let color = temp[0]['color']//0%处的颜色 中心
                            let edgeColor = temp[0]['edgeColor']//100%处的颜色 边缘
